...
describe('lockedDecision (TR-36, SR-12)', () => {
  test('locks nothing before any decision reaches 3 of 5', () => {
    assert.equal(lockedDecision(emptyBuffer), null);
    assert.equal(lockedDecision(pushAll([accept('a'), accept('a')])), null);
  });

  test('locks when 3 of the last 5 agree, even with noise between them', () => {
    const locked = lockedDecision(pushAll([accept('a'), accept('b'), accept('a'), unknown, accept('a')]));
    assert.deepEqual(locked, accept('a'));
  });

  test('returns the most recent agreeing decision, so the confidence shown is current', () => {
    const locked = lockedDecision(pushAll([accept('a', 0.625), accept('a', 0.75), accept('a', 0.875)]));
  ...
  test('a disambiguation pair agrees in either order', () => {
    const locked = lockedDecision(pushAll([disambiguate('a', 'b'), disambiguate('b', 'a'), disambiguate('a', 'b')]));
    assert.equal(locked?.kind, 'disambiguate');
  });

  test('repeated Unknown locks as Unknown (SR-04)', () => {
    assert.deepEqual(lockedDecision(pushAll([unknown, unknown, unknown])), unknown);
  });

  test('old decisions slide out of the window and release the lock', () => {
    const buffer = pushAll([accept('a'), accept('a'), accept('a'), accept('c'), accept('d'), accept('e')]);

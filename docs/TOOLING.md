...
**The toolchain is now proven end to end** (2026-09-13): NDK 27.1.12297006 is installed, the debug
...
dev server still reported "Waiting on http://localhost:8081". It is now an explicit
devDependency. It is a build-time preset and never ships in the APK, so `TR-50`/`TR-51` are
unaffected.
